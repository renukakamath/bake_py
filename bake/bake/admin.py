...
@admin.route('/purchase_details',methods=['post','get'])
def admin_purchase_details():
    data={}
...
@admin.route('/admin_assign',methods=['post','get'])
def	admin_assign():
    data={}
    ...
    if "update"in request.form:
        ...
        update(q)
        return redirect(url_for('admin.admin_viewbooking'))
    return render_template('admin_assign.html',data=data)



@admin.route('/admin_customerreport')
